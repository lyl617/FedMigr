...
parser.add_argument('--checkpoint_dir', type=str, default='./Checkpoint_tmp/',
                    help='')
parser.add_argument('--pattern_idx', type=int, default=3,
                    help='')
...
if __name__ == "__main__":
    ...
    # pattern_list = ['bias', 'partition', 'random', 'in_cluster','x_cluster','r_cluster']
    ...
    LOAD_MODEL_PATH = checkpoint_dir + 'MIGR_GPU' + '_' + args.dataset_type + '_' + args.model_type + '_vm' + str(args.vm_num) + '_' + pattern_list[pattern_idx] + 'data' + \
    ...
    SAVE_MODEL_PATH = checkpoint_dir + 'MIGR_GPU' + '_' + args.dataset_type + '_' + args.model_type + '_vm' + str(args.vm_num) + '_' + pattern_list[pattern_idx] + 'data' + \
        '_lr' + str(args.lr) + '_minlr' + str(args.min_lr) + '_decay' + str(args.enable_lr_decay) + '_epoch' + str(args.epochs) + \
    ...
    LOG_PATH = checkpoint_dir + 'MIGR_GPU' + '_' + args.dataset_type + '_' + args.model_type + '_vm' + str(args.vm_num) + '_' + pattern_list[pattern_idx] + 'data' + \
        '_lr' + str(args.lr) + '_minlr' + str(args.min_lr) + '_decay' + str(args.enable_lr_decay) + '_epoch' + str(args.epochs) + \
    ...
    FIG_ROOT_PATH = fig_dir + 'MIGR_GPU' + '_' + args.dataset_type + '_' + args.model_type + '_vm' + str(args.vm_num) + '_' + pattern_list[pattern_idx] + 'data' + \
        '_lr' + str(args.lr) + '_minlr' + str(args.min_lr) + '_decay' + str(args.enable_lr_decay) + '_epoch' + str(args.epochs) + \
    ...
    for epoch in range(epoch_start + 1, epoch_end + 1):
        ...
        if epoch == epoch_start + 1 or epoch % args.aggregate_interval == 1 or args.aggregate_interval == 1:
            ...
            with torch.no_grad():
                ...
                if pattern_idx < 4 or pattern_idx == 6:
                    vm_random_idxs = list(range(args.vm_num))
                    random.shuffle(vm_random_idxs)
                # [Debuge begin]
                elif pattern_idx == 4:
                    vm_random_idxs1 = [0,1,2,3,4]
                    vm_random_idxs2 = [5,6,7,8,9]
                    random.shuffle(vm_random_idxs1)
                    random.shuffle(vm_random_idxs2)
                    vm_random_idxs = vm_random_idxs1 + vm_random_idxs2
                elif pattern_idx == 5:
                    vm_random_idxs1 = [0,1,2,3,4]
                    vm_random_idxs2 = [5,6,7,8,9]
                    random.shuffle(vm_random_idxs1)
                    random.shuffle(vm_random_idxs2)
                    vm_random_idxs = vm_random_idxs2 + vm_random_idxs1
                # [Debug end]

                printer('--[{}] random idxs: {}'.format(time_since(start), vm_random_idxs), log_out)
                if pattern_idx == 1:
